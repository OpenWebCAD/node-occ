...
describe("testing face mesh ", function () {
  it("Face#mesh - should not have a mesh unless the parent solid has been meshed", function () {
    ...
    // now mesh the solid
    let m = solid.mesh;
    m.vertices.length.should.eql(
      8 * 3,
      "expecting 8 vertices (made of 3 floats) in solid mesh"
    );
    ...
    topFace.hasMesh.should.equal(
      true,
      "Face must have a mesh when parent solid is meshed"
    );
...
describe("testing face#getWire ", function () {
  it("Face#getWire", function () {
    ...
    wires[0]
      .getEdges()[0]
      .getVertices()[0]
      .should.eql(new Vertex({ x: 0, y: 10, z: 10 }));
  });
});
