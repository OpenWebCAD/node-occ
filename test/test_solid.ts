...
import { EdgeIterator, FaceIterator, IBoundingBox, IEdge, IFace, IShapeIterator, ISolid, Solid, Triplet, occ } from "..";
...
describe("testing solid construction", () => {
    ...
    describe("testing ShapeIterator on solid", () => {
        let solid: ISolid;
        let shapeIt: IShapeIterator<ISolid> | IShapeIterator<IFace> | IShapeIterator<IEdge>;
        before(() => {
            solid = occ.makeBox([10, 20, 30], [20, 40, 60]);
        });
    ...
    describe("makeCylinder (variation 2)", () => {
        let solid: ISolid;
        before(() => {
            let position: [Triplet, Triplet] = [[0, 0, 1], [0, 1, 0]];
            let radius = 50;
            let height = 100;
