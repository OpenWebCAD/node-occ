import "should";
import { IMesh, ISolid, Real, occ, shapeFactory } from "..";
...
describe("TBugLinux- testing mesh on a simple cone shape with radius2 = 0 returns 2 Faces (latteral+bottom)", function () {
  ...
  it("solid should have 2 faces", function () {
    ...
    shape.hasMesh.should.be.eql(true);
    shape.faces["lateral"].hasMesh.should.be.eql(true);
    shape.faces["bottom"].hasMesh.should.be.eql(true);
  });
});
...
describe("T1- testing mesh on a simple box shape", function () {
  ...
  it("Mesh#vertices          - mesh should provide a vertex array", function () {
    ...
    mesh.vertices[8].should.eql(0.0);

    mesh.vertices[9].should.eql(0.0);
    mesh.vertices[10].should.eql(40.0);
    mesh.vertices[11].should.eql(100.0);
  ...
  it("Mesh#triangle          - mesh should provide triangles indexes", function () {
    // --------------------------------------- triangles
    mesh.triangles.length.should.eql(
      6 * 2 * 3,
      "it should provide 2 triangles per face = 2 *4 *3"
    );
  });
  it("Mesh#faceRanges        - mesh should provide a mechanism to easily identify triangle faces", function () {
    mesh.faceRanges.length.should.eql(
      6 * 2,
      "it should 6 pairs of index ( 6 faces, 2 values per face)"
    );
    mesh.edgeRanges.length.should.eql(
      12 * 2,
      "it should 12 pairs of index ( 12 edges, 2 values per face)"
    );
...
describe("testing performance of meshing algorithms with various parameters", function () {
  ...
  function makeLegoBrick() {
    return shapeFactory.makeLegoBrick(occ, 4, 2, "thin");
  }

  function installFor(name: string, makeShape: () => ISolid) {
    let shape2: ISolid;
    beforeEach(function () {
      shape2 = makeShape();
    });

    function test_with(tol: Real, angle: Real) {
      it(
        name +
          " testing with parameter : deflection : " +
          tol +
          "  angle :" +
          angle,
        function () {
