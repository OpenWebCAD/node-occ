...
import { OCC, IEdge, IFace, IPoint, PointLike, Triplet, UInteger, VectorLike } from "./interfaces";
...
export function makePan(csg: OCC, _height: number, _radius: number) {
    ...
    const pt2: Triplet = [handleLength + radius - 2 * thickness, 0, height - handleRadius * 1.1];
 ...
 */
export function makeCylinderTorusCone(csg: OCC, r1: number, r2: number, H: number, H2: number, rs: number) {
    // ------------------------------------------------------------
 ...
 *                       +
 */
export function makeLidWithTorus(csg: OCC, r: number, h: number, rs: number, thickness: number) {
...
export function makeTube(csg: OCC, p1: PointLike, p2: PointLike, R: number, thickness: number) {
...
export function makeHollowCylinder(csg: OCC, R: number, H: number, h: number, rf: number, thickness: number) {
...
export function makeLegoBrickSlow(csg: OCC, nX: UInteger, nY: UInteger, type: "thin" | "thick") {
...
function makeRepetition<T extends { clone(): T, translate(v: VectorLike): T }>(csg: OCC, shape: T, dX: number, nX: number, dY: number, nY: number) {
    let h1 = shape.clone();
    // installer la grille
    const shapeArray: T[] = [];
    for (let y = 0; y < nY; y++) {
...
export function makeLegoBrick(csg: OCC, nX: UInteger, nY: UInteger, type: "thin" | "thick") {
...
export function makeTutorialPart(occ: OCC) {
    ...
    function selectEdge(edges: IEdge[], p: PointLike) {
        ...
        const results = edges.filter((edge) => {
            ...
            return (samePoint(firstVertex, p as IPoint) || samePoint(lastVertex, p as IPoint)) &&
