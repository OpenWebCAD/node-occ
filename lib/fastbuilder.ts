...
class Cache {
    ...
    replaceObjectMethod(obj: IObject, _methodName: string) {
        ...
        function performOperation(this: IObject, ...args: any[]) {
            return that.__performedCachedOperation(this, originalMethod, `${methodName}#${this.uuid}`, args, false);
        }
    ...
    prepareObject(obj: IObject, cmd: string, uuid: string, index?: number | string) {
    ...
    __performedCachedOperation(originalObject: IObject, originalMethod: ObjectMethod, methodName: string, theArguments: any[], callAsConstructor: boolean) {
        ...
        try {
            ...
            debugLog(` FAILING to call ${methodName}(`, `${theArguments.map((e) => JSON.stringify(theArguments[e])).join(" , ")})`);
    ...
    __proxy(originalObject: IObject, methodName: string, theArguments: any[], callAsConstructor: boolean) {
...
function replaceObjectsByGUUID(arg: any) {
    ...
    if (arg.hasOwnProperty("uuid")) {
        ...
        return arg;
    } else if ((arg.length > 0 && typeof arg[0] === "string") || (typeof arg === "string")) {
        return arg;
 ...
 * @private
 */
export function calculateOperationHash(methodName: string, theArguments: any[], _callAsConstructor: boolean) {
...
function replaceFunc(cache: Cache, proxyObj: IObject, Obj: IObject, methodName: string) {
    assert(proxyObj);
...
export function createProxyObject<IObject>(originalObject: IObject): IObject {
    ...
    properties.forEach((funcName) => replaceFunc(cache, proxyObject, originalObject, funcName));
