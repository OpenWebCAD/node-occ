...
exports.makeBottle = function (occ, parameters)
{
    ...
    var aSegment1 = occ.makeLine(aPnt1, aPnt2);
    var aArc1 = occ.makeArc3P(aPnt2, aPnt3, aPnt4);
    var aSegment2 = occ.makeLine(aPnt4, aPnt5);
...
//
    ...
    var e1 = occ.makeLine(q1, p1);
    var e2 = occ.makeLine(p1, p2);
    var e3 = occ.makeLine(p2, p3);
    var e4 = occ.makeArc3P(p3, p4, q3);
    var e5 = occ.makeLine(q3, q2);
    var e6 = occ.makeLine(q2, q1);
...
exports.makeTutorialPart = function (occ) {
    ...
    var e1 = occ.makeLine(p0, p1);
    var e2 = occ.makeLine(p1, p2);
    var e3 = occ.makeLine(p2, p3);
    var e4 = occ.makeLine(p3, p4);
    var e5 = occ.makeLine(p4, p5);
    var e6 = occ.makeLine(p5, p6);
    var e7 = occ.makeLine(p6, p7);
    var e8 = occ.makeArc3P(p7, p8, p0);

    var wire = occ.makeWire(e1, e2, e3, e4, e5, e6, e7, e8);
    wire.isClosed.should.eql(true);
    console.log(" wire Length = ", wire.length);

    var height = 20;
    var face = occ.makeFace(wire);
    var body1 = occ.makePrism(face, [0, 0, height]);

    // --------------------------------------------------
    var height2 = 45;
    var circle = occ.makeCircle([w, 0, 0], [0, 0, 1], h);
    var wire2 = occ.makeWire(circle);
    var face2 = occ.makeFace(wire2);
    var body2 = occ.makePrism(face2, [0, 0, height2]);

    // -----------------------------------------------------
    var body3 = occ.fuse(body1, body2);
    ...
    var R = 15;
    var c = 7.5;
    var angle = Math.asin(7.5 / R);
    var dx = R * Math.cos(angle);
    var dy = R * Math.sin(angle);

    var q1 = [w + dx, dy, 0];
    var q2 = [w + dx + 100, dy, 0];
    var q3 = [w + dx + 100, -dy, 0];
    var q4 = [w + dx, -dy, 0];
    var q5 = [w - R, 0, 0];

    var ee1 = occ.makeLine(q1, q2);
    var ee2 = occ.makeLine(q2, q3);
    var ee3 = occ.makeLine(q3, q4);
    var ee4 = occ.makeArc3P(q4, q5, q1);
    var wire4 = occ.makeWire(ee1, ee2, ee3, ee4);
    var face4 = occ.makeFace(wire4);
    var body4 = occ.makePrism(face4, [0, 0, height2]);

    var body5 = occ.cut(body3, body4);


    var edges = body5.getEdges();
    // --------------------------------------------
    ...
    function selectEdge(edges, p) {
        ...
        var results = edges.filter(function (edge) {
            var firstVertex = edge.firstVertex;
            var lastVertex = edge.lastVertex;
    ...
    var edges_for_filet = [selectEdge(edges, p2), selectEdge(edges, p5)];

    var body6 = occ.makeFillet(body5, edges_for_filet, 10);

    // create hole
    var smallR = 5;
    var heigth3 = height2 - smallR - 10;
    var cyl = occ.makeCylinder([w - R - 10, 0, heigth3], [w - R + 20, 0, heigth3], smallR);

    var body7 = occ.cut(body6, cyl);
