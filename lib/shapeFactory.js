...
exports.makeLidWithTorus = function(csg,r,h,rs,thickness) {
    ...
    //    .   | tan(a)=s/c => s=h*tan(
    //  +-----|-------
    let c1 = [center[0], center[1], center[2] + hh];
    let r1 = Math.tan(2 * alpha) * hh;
    let c2 = [center[0], center[1], center[2] + (R + hh)];
    let r2 = Math.tan(2 * alpha) * (R + hh);

    let cuttingcone = csg.makeCone(c1, r1, c2, r2);
