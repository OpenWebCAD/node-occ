...
exports.makeBottle = function ()
{
    ...
    myBody.getFaces().forEach(function(face){
         console.log(" examining  face = ", myBody.getShapeName(face),face.isPlanar,face.centreOfMass.z);
        if (face.isPlanar) {
    ...
    console.log(" write top face = ", myBody.getShapeName(faceToRemove));
...
occ.makeCylinderTorusCone = function(r1,r2,H,H2,rs)
{
	...
	var r1b = r1 -rs*(1.-Math.cos(angle));
...
{
    var top =  makeLidWithTorus(csg,R,h,rf,thickness);
    var bottom = top.clone();
    bottom = bottom.rotate([0,0,0],[1,0,0],180);
    
    var cyl = makeTube(csg,[0,0,0],[0,0,H],R,thickness);
...
exports.testHollowCylinder= function() {
