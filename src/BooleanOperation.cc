...
BooleanOperation::BooleanOperation() : m_bop(0) {}
BooleanOperation::~BooleanOperation() { delete m_bop; }
...
BOPAlgo_Operation ReadOperationType(const v8::Local<v8::Value> &arg) {
  ...
  if (_isEqual(str, "COMMON"))
    return BOPAlgo_COMMON;
  if (_isEqual(str, "FUSE"))
    return BOPAlgo_FUSE;
  if (_isEqual(str, "CUT"))
    return BOPAlgo_CUT;
  if (_isEqual(str, "CUT21"))
...
NAN_METHOD(BooleanOperation::New) {
  if (!info.IsConstructCall()) {
    return Nan::ThrowError(
        " use new occ.BooleanOperation() to construct a BooleanOperation");
  }
  ...
  if (op == BOPAlgo_UNKNOWN) {
    return Nan::ThrowError(
        "bad operation type, must be SECTION COMMON FUSE CUT or CUT21");
  }
...
void BooleanOperation::Init(v8::Local<v8::Object> target) {
  ...
  Nan::Set(target, Nan::New("BooleanOperation").ToLocalChecked(),
