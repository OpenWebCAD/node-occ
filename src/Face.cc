...
Face::~Face() { m_cacheMesh.Reset(); };

const TopoDS_Shape &Face::shape() const { return face(); }

void Face::setShape(const TopoDS_Shape &shape) { m_face = TopoDS::Face(shape); }
...
bool Face::fixShape() { return true; }
...
bool Face::hasMesh() {
  ...
  occHandle(Poly_Triangulation) triangulation =
      BRep_Tool::Triangulation(this->face(), loc);
...
v8::Local<v8::Object> Face::createMesh(double factor, double angle,
                                       bool qualityNormals) {
  ...
  try {
    ...
    if (triangulation.IsNull()) {
      BRepMesh_IncrementalMesh MSH(shape, factor, Standard_True, angle,
                                   Standard_True);
    }
...
void Face::Init(v8::Local<v8::Object> target) {
  ...
  Nan::Set(target, Nan::New("Face").ToLocalChecked(),
