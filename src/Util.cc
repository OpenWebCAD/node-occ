...
void ReadPropertyPointFromArray(v8::Local<v8::Array> arr, double *x, double *y,
                                double *z) {
...
// void ReadPropertyPoint(Handle<Object> obj,const char* name,double* x,double*
// y, double*z )
//{
//  if (!obj.IsEmpty()) {
...
void ReadInt(v8::Local<v8::Object> value, const char *name, int *retValue,
             int defaultValue) {
  Nan::HandleScope scope;
...
void ReadPoint(v8::Local<v8::Value> value, double *x, double *y, double *z) {
  ...
  if (value->IsArray()) {
    ...
    Nan::ThrowError(
        "Invalid Point or Vector ( must be a [] or a {x:..,y:.., z:...} )");
  }
}
...
void ReadRotationFromArgs(_NAN_METHOD_ARGS, gp_Trsf &trans) {
  ...
  trans.SetRotation(gp_Ax1(gp_Pnt(x, y, z), gp_Dir(u, v, w)),
                    angle / 180.0 * M_PI);
}
