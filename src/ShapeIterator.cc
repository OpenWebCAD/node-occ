...
void ShapeIterator::reset() { ex.ReInit(); }
...
NAN_MODULE_INIT(ShapeIterator::Init) {
  ...
  Nan::Set(target, Nan::New("ShapeIterator").ToLocalChecked(),
...
NAN_METHOD(ShapeIterator::New) {
  ...
  if (!info.IsConstructCall()) {
    return Nan::ThrowError(
        " use new occ.ShapeIterator() to construct a ShapeIterator");
  }

  if (info.Length() != 2) {
    return Nan::ThrowError(" expecting two arguments : "
                           "<Shape>,<'VERTEX'|'WIRE'|'SOLID'|'FACE'...>");
  }

  // TODO (check that the object info[0] has the correct type)
  Base *pShape = Nan::ObjectWrap::Unwrap<Base>(
