...
NAN_METHOD(Mesh::New) {
  ...
  Nan::Set(info.This(), Nan::New("normals").ToLocalChecked(),
           Nan::New<v8::Object>());
  Nan::Set(info.This(), Nan::New("edgeIndices").ToLocalChecked(),
           Nan::New<v8::Object>());
  Nan::Set(info.This(), Nan::New("triangles").ToLocalChecked(),
           Nan::New<v8::Object>());
...
NAN_MODULE_INIT(Mesh::Init) {
  ...
  Nan::Set(target, Nan::New("Mesh").ToLocalChecked(),
...
int Mesh::extractEdgeFromFace(const TopoDS_Face &face,
                              const occHandle(Poly_Triangulation) &
                                  triangulation,
                              const std::vector<uint32_t> &translationMap) {
...
int Mesh::extractEdge(const TopoDS_Edge &edge,
  ...
  TopLoc_Location loc;
  occHandle(Poly_PolygonOnTriangulation) edgepoly =
      BRep_Tool::PolygonOnTriangulation(edge, triangulation, loc);
  if (edgepoly.IsNull()) {
    return 0;
  }
...
template <class T>
void UpdateExternalArray(v8::Local<v8::Object> &pThis, const char *name,
                         const T *data, size_t _length) {
...
void Mesh::updateJavaScriptArray() {
  ...
  UpdateExternalArray(pThis, "vertices", &_vertices.data()[0].x,
                      _vertices.size() * 3);
  UpdateExternalArray(pThis, "normals", &_normals.data()[0].x,
                      _normals.size() * 3);
  UpdateExternalArray(pThis, "triangles", &_triangles.data()[0].i,
                      _triangles.size() * 3);
  UpdateExternalArray(pThis, "triangleNormals", &_triangles_normals.data()[0].i,
                      _triangles_normals.size() * 3);

  UpdateExternalArray(pThis, "faceRanges", &_faceRanges.data()[0],
                      _faceRanges.size());
  UpdateExternalArray(pThis, "edgeIndices", &_edgeIndices.data()[0],
                      _edgeIndices.size());
  UpdateExternalArray(pThis, "edgeRanges", &_edgeRanges.data()[0],
                      _edgeRanges.size());
}
...
template <class uintX_t>
void makeTT(const std::vector<uint32_t> &arr, uint32_t start, uint32_t length,
            std::vector<uintX_t> &indexes) {
  indexes.clear();
...
NAN_METHOD(Mesh::getFaceTriangles) {
  ...
  if (info.Length() == 1 && info[0]->IsObject()) {
    pFace = Nan::ObjectWrap::Unwrap<Face>(
...
NAN_METHOD(Mesh::getEdgeIndices) {
  ...
  Edge *pEdge = 0;
  if (info.Length() == 1 && info[0]->IsObject()) {
    pEdge = Nan::ObjectWrap::Unwrap<Edge>(
        Nan::To<v8::Object>(info[0]).ToLocalChecked());
  }
  if (!pEdge) {
...
int deprecated_findPt(const std::vector<Coord3f> &arrayPts, const Coord3f pt,
                      double tolerance) {

  double tolerance2 = tolerance * tolerance;
  int i = 0;
  for (std::vector<Coord3f>::const_iterator it = arrayPts.begin();
       it != arrayPts.end(); it++) {
...
void Mesh::optimize() { updateJavaScriptArray(); }
// see also :
