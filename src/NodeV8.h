...
inline v8::Local<v8::String> c(Nan::MaybeLocal<v8::String> e) {
  return e.ToLocalChecked();
}
...
#define EXPOSE_METHOD(ClassName, staticMethod)                                 \
  Nan::SetTemplate(proto, #staticMethod,                                       \
                   Nan::New<v8::FunctionTemplate>(staticMethod))
...
#define __EXPOSE_READ_ONLY_PROPERTY(element, staticMethod, name)               \
  Nan::SetAccessor(element, Nan::New(#name).ToLocalChecked(), &staticMethod,   \
                   0, v8::Local<v8::Value>(), v8::DEFAULT, v8::ReadOnly)
...
#define EXPOSE_READ_ONLY_PROPERTY_BOOLEAN(ClassName, name)                     \
  __EXPOSE_READ_ONLY_PROPERTY(                                                 \
      proto, (ee<ClassName, v8::Boolean, bool, &ClassName::name>), name)

#define EXPOSE_READ_ONLY_PROPERTY_INTEGER(ClassName, name)                     \
  __EXPOSE_READ_ONLY_PROPERTY(                                                 \
      proto, (ee<ClassName, v8::Integer, int, &ClassName::name>), name)

#define EXPOSE_READ_ONLY_PROPERTY_UINT32(ClassName, name)                      \
  __EXPOSE_READ_ONLY_PROPERTY(                                                 \
      proto, (ee<ClassName, v8::Integer, uint32_t, &ClassName::name>), name)

#define EXPOSE_READ_ONLY_PROPERTY_DOUBLE(ClassName, name)                      \
  __EXPOSE_READ_ONLY_PROPERTY(                                                 \
      proto, (ee<ClassName, v8::Number, double, &ClassName::name>), name)

#define EXPOSE_READ_ONLY_PROPERTY_CONST_STRING(ClassName, name)                \
  __EXPOSE_READ_ONLY_PROPERTY(                                                 \
      proto, (ee<ClassName, v8::String, const char *, &ClassName::name>),      \
      name)

#define EXPOSE_READ_ONLY_PROPERTY_OBJECT(ClassName, name)                      \
  __EXPOSE_READ_ONLY_PROPERTY(                                                 \
      proto,                                                                   \
      (ee<ClassName, v8::Value, v8::Local<v8::Object>, &ClassName::name>),     \
      name)

#define REXPOSE_READ_ONLY_PROPERTY_DOUBLE(ClassName, name)                     \
  __EXPOSE_READ_ONLY_PROPERTY(                                                 \
      info.This(), (ee<ClassName, v8::Number, double, &ClassName::name>),      \
      name)
#define REXPOSE_READ_ONLY_PROPERTY_BOOLEAN(ClassName, name)                    \
  __EXPOSE_READ_ONLY_PROPERTY(                                                 \
      info.This(), (ee<ClassName, v8::Boolean, bool, &ClassName::name>), name)
#define REXPOSE_READ_ONLY_PROPERTY_INTEGER(ClassName, name)                    \
  __EXPOSE_READ_ONLY_PROPERTY(                                                 \
      info.This(), (ee<ClassName, v8::Integer, int, &ClassName::name>), name)
#define REXPOSE_READ_ONLY_PROPERTY_CONST_STRING(ClassName, name)               \
  __EXPOSE_READ_ONLY_PROPERTY(                                                 \
      info.This(),                                                             \
      (ee<ClassName, v8::String, const char *, &ClassName::name>), name)
/**
...
#define CATCH_AND_RETHROW(message)                                             \
    ...
    std::cerr << "C++ exception in OCC " << msg << " " << message              \
              << std::endl;                                                    \
