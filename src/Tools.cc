...
NAN_METHOD(writeGLTF)
{
  ...
  occHandle(TDocStd_Document) doc = new TDocStd_Document(TCollection_ExtendedString("XmlOcaf"));
  ...
  auto ret = writer.Perform(doc, TColStd_IndexedDataMapOfStringString(), Message_ProgressRange());

  info.GetReturnValue()
      .Set(Nan::New<v8::Boolean>(true));
}
