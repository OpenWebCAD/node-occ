...
v8::Local<v8::Value> BoundingBox::NewInstance(const gp_Pnt &nearPt,
                                              const gp_Pnt &farPt) {
  Bnd_Box box;
  box.Add(nearPt);
  box.Add(farPt);
...
NAN_METHOD(BoundingBox::New) {
  if (!info.IsConstructCall()) {
    return Nan::ThrowError(
        " use new occ.BoundingBox() to construct a BoundingBox");
  }
...
NAN_MODULE_INIT(BoundingBox::Init) {
  ...
  Nan::Set(target, Nan::New("BoundingBox").ToLocalChecked(),
