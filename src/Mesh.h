...
public:
  ...
  uint32_t numTriangles() { return (uint32_t)_triangles.size(); }

  int32_t numVertices() { return (int32_t)_vertices.size(); }

  int32_t numNormals() { return (int32_t)_normals.size(); }

  int32_t numEdges() { return (int32_t)_edgeIndices.size() >> 1; }
