...
void Solid::Init(v8::Local<v8::Object> target) {
  ...
  Nan::Set(target, Nan::New("Solid").ToLocalChecked(),
...
NAN_METHOD(Solid::New) {
  ...
  Nan::DefineOwnProperty(
      info.This(), Nan::New("faces").ToLocalChecked(), Nan::New<v8::Object>(),
      (v8::PropertyAttribute)(v8::DontDelete | v8::ReadOnly));

  Nan::DefineOwnProperty(
      info.This(), Nan::New("_reversedMap").ToLocalChecked(),
      Nan::New<v8::Object>(),
      (v8::PropertyAttribute)(v8::DontEnum | v8::DontDelete | v8::ReadOnly));

  /// args.This()->SetAccessor(NanSymbol("_area"),ee< Solid, Number, double,
  /// &Solid::area>,0,Number::New(12),DEFAULT,None);
...
NAN_METHOD(Solid::getCommonEdges) {
  ...
  if (info.Length() < 2 || !extractArg(info[0], pFace1) ||
      !extractArg(info[1], pFace2)) {
    return Nan::ThrowError(
        "invalid arguments getCommonEdges : expecting <FACE>,<FACE>");
  }
  ...
  for (; edgeExplorer.More(); edgeExplorer.Next()) {
    ...
    int nbFound = 0;
    for (; it.More(); it.Next()) {
      if (pFace1->shape() == it.Value()) {
        nbFound++;
        continue;
      }
      if (pFace2->shape() == it.Value()) {
        nbFound++;
        continue;
      }
    }
...
const char *getCommonVertices_Doc =
    "Solid.getCommonVertices(<Face>,<Face>);\n"
...
bool Solid::hasMesh() { return !m_cacheMesh.IsEmpty(); }
...
//    }
//}
v8::Local<v8::Object> Solid::createMesh(double factor, double angle,
                                        bool qualityNormals) {
  ...
  try {
    ...
    if (angle == 0.0) {
      ...
      BRepMesh_IncrementalMesh m2(shape, factor, Standard_True, angle,
                                  Standard_True);
    }
...
NAN_METHOD(Solid::getShapeName) {
  ...
  if (!pShape.IsEmpty()) {
    v8::Local<v8::Value> hashCode =
        Nan::Get(pShape, Nan::New("hashCode").ToLocalChecked())
            .ToLocalChecked();

    auto _rm = Nan::Get(pJhis, Nan::New("_reversedMap").ToLocalChecked())
                   .ToLocalChecked();
    v8::Local<v8::Object> reversedMap =
        Nan::To<v8::Object>(_rm).ToLocalChecked();

    v8::Local<v8::Value> value =
        Nan::Get(reversedMap, hashCode).ToLocalChecked();
...
std::string Solid::_getShapeName(const TopoDS_Shape &shape) {
  ...
  v8::Local<v8::Value> hashCode =
      Nan::New<v8::Integer>(shape.HashCode(std::numeric_limits<int>::max()));
...
void Solid::_registerNamedShape(const char *name, const TopoDS_Shape &shape) {
  if (shape.ShapeType() == TopAbs_FACE) {
    ...
    v8::Local<v8::Object> obj =
        Nan::To<v8::Object>(Nan::Get(NanObjectWrapHandle(this),
                                     Nan::New("faces").ToLocalChecked())
                                .ToLocalChecked())
            .ToLocalChecked();

    Nan::Set(obj, Nan::New(name).ToLocalChecked(),
             Face::NewInstance(TopoDS::Face(shape)));
  }
  ...
  Nan::Set(reversedMap, shape.HashCode(std::numeric_limits<int>::max()),
